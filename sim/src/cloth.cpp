...
void Cloth::burgers_flow(double delta_t) {
    ...
    flux.resize(num_vertices);
    areas.resize(num_vertices);

    double coeff = 1.0 / 8.0 / PI;
    double d_ave = 0;
    for (int i = 0; i < num_vertices; i++) {
        int i_plus1 = (i + 1) % num_vertices;
        PointMass& pm0 = point_masses.at(i);
        PointMass& pm1 = point_masses.at(i_plus1);
        Vector3D T = pm1.position - pm0.position;
        double d = T.norm();
        T = T.unit();
        d_ave += d;
        pm0.length = d;
        areas(i) = PI * pm0.thickness * pm0.thickness;
        pm0.grav = dot(At * g, T);
    }
    d_ave = d_ave / num_vertices;
    ...
    for (int i = 0; i < num_vertices; i++) {
        ...
        double pre = areas(i) * pm0.grav;
        double nex = areas(i_plus1) * pm1.grav;

        if (pre > max(0.0, -nex)) {
            flux(i) = coeff * pm0.grav * areas(i) * areas(i);
        }
        else if (nex < min(0.0, -pre)) {
            flux(i) = coeff * pm1.grav * areas(i_plus1) * areas(i_plus1);
        }
        else {
            flux(i) = 0.0;
        }
    ...
    //Eigen::MatrixXd::Identity(num_vertices, num_vertices);
    Eigen::MatrixXd graph;
    Eigen::MatrixXd M;
    Eigen::MatrixXd st;
    graph.resize(num_vertices, num_vertices);
    for (int i = 0; i < num_vertices; i++) {
        int i_plus1 = (i + 1) % num_vertices;
        graph(i, i) = -1.0;
        graph(i, i_plus1) = 1.0;
        ...
        PointMass& pm0 = point_masses.at(i);
        M(i, i) = pm0.length;
        st(i, i) = pm0.circulation * pm0.circulation / d_ave;
    }
    Eigen::MatrixXd graphT = graph.transpose();
    Eigen::MatrixXd L = -graphT * st * graph;

    double coeff2 = 1.0 / 64.0 / PI / PI;
    double nudt = nu / delta_t;

    Eigen::MatrixXd LHS = nudt * M - 0.5 * coeff2 * L;
    Eigen::MatrixXd RHS = nudt * M * areas + graphT * flux;

    Eigen::VectorXd newA = LHS.lu().solve(RHS);
    cout << newA;
